...
# @torch.compile() # removed for compatibility
def zeropower_polar_express(
    G: torch.Tensor,
    steps: int = 5,
):
    ...
    # Ensure steps don't exceed available coeffs
    steps = min(steps, len(coeffs_list))
    ...
    # Fixed: added keepdim=True and proper normalization
    ...
    def step(self):
        for group in self.param_groups:
            for p in group["params"]:
                ...
                g = zeropower_polar_express(
                    g, steps=group["ns_steps"]
                )  # steps are 5 for both ns and pe
                g = g.to(p.dtype)
                if p.size(-1) >= 1:
                    p.add_(
                        g.view_as(p),
